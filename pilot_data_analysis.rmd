...
s1_filelist <- list.files(path="~/Downloads", pattern="*.csv", full.names=T)
...
for (t in 1:(nrow(learn_data)-1)){
  ...
  for (i in 1:4){
    ...
    if(length(which(learn_data$chosen_door_num[(1+(learn_data$ID[t]-1)*150):t] %in% i))==0){
      exploration_bonus[t+1,i] <- t - (learn_data$ID[t]-1)*150 # hasn't been chosen before
    }else{
      exploration_bonus[t+1,i] <- t - (max(which(learn_data$chosen_door_num[(1+(learn_data$ID[t]-1)*150):t] %in% i)) + (learn_data$ID[t]-1)*150)
    }
  }
}
...
pilot_fit_delta_SMEPR <- stan(
  file = "delta_SMEPR_1sub.stan",  # Stan program
...
traceplot(pilot_fit_delta_SMEPR, pars = c("beta","alpha","phi","persev","gamma"))
stan_hist(pilot_fit_delta_SMEPR, pars = c("beta","alpha","phi","persev","gamma"))
...
Model derived expected values, directed exploration bonus (uncertainty), choice probabilities for the 4 doors:
...
# expected values
Q <- fit_summary %>%
  select(-c(alpha,beta,phi,persev,gamma,lp__)) %>%
  slice(1) %>%
  as.matrix() %>%
  matrix(nrow = 4, ncol = 150) %>%
  t() %>%
...
points(reward_schedule$V1, col=door_key[1])
points(reward_schedule$V2, col=door_key[2])
points(reward_schedule$V3, col=door_key[3])
points(reward_schedule$V4, col=door_key[4])
```
