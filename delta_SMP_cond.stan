...
// and a standard softmax choice rule with an additional perservaration
...
model {
  ...
  for (t in 1:totalTrials){
    ...
    if(trialNum[t]>1){
      ...
      choices[t] ~ categorical_logit(beta[subject[t]] * Q[t] + pb); // the probability of the choices on each trial given utilities and exploration bonus
  }
}
...
generated quantities{
  ...
  for (t in 1:totalTrials){
    ...
    if(trialNum[t]>1){
      ...
      log_lik[t] = categorical_logit_lpmf(choices[t] | (beta[subject[t]] * Q[t] + pb));
  }
