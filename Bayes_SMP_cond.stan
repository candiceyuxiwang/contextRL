...
//  vector[4] pb;  // perseveration bonus
  ...
  // actual beta and persev parameters, determined by mu and sigma hyperparameters
  ...
  for (t in 1:totalTrials){
    vector[4] pb;  // perseveration bonus
      ...
      if (trialNum[t]>1) {
        ...
        choices[t] ~ categorical_logit(beta[subject[t]] * v[t] +  pb);
  }
...
generated quantities{
  ...
  for (t in 1:totalTrials){
   ...
   log_lik[t] = categorical_logit_lpmf(choices[t] | beta[subject[t]] * v[t] +  pb);
  }
