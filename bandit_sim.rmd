...
for (t in 1:nrow(sim_data)){
  sim_data$optimal[t] <- which.max(reward_schedule[sim_data$trialnum[t],])==sim_data$choice[t]
}
...
From the plot, it is clear that simulated subjects generally perform very well in stretches of trials where one option is clearly superior than others (e.g. trial bin #7). In contrast, when two or more options have similar reward values, the probability of choosing the optimal arm can be lower for some or all subjects. 

Likelihood of exploratory choices in a given period of the task depend on the reward schedule
